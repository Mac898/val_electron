...
fn setup(
    ...
    inventory.set_slot(0, ItemStack::new(ItemKind::Redstone, 1, None));
    inventory.set_slot(1, ItemStack::new(ItemKind::DiamondPickaxe, 1, None));

    inventory.set_slot(
        2,
        ItemStack::new(ItemKind::DiamondPickaxe, 1, Some(Compound::new())),
    );

    let mut nbt = Compound::new();
    nbt.insert("Damage", Int(0));
    inventory.set_slot(3, ItemStack::new(ItemKind::DiamondPickaxe, 1, Some(nbt)));
...
pub fn handle_item_click(_: Commands, mut events: EventReader<ClickSlotEvent>) {
    for event in events.iter() {
        //println!("Event: {:#?}", event);
    }
}
